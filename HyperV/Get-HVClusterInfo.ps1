Write-Host -ForegroundColor Yellow "This script needs to be run from one of the Hyper-V Cluster nodes!"

$outPath = Read-Host "Please enter Full output path (.csv extension)"

$vmOutput = New-Object System.Collections.ArrayList

Write-Host -foregroundcolor Cyan "Gathering VM info from cluster '$((Get-Cluster).Name)' and writing it to '$outPath'"
Write-Host -ForegroundColor Yellow "This may take a few moments, please wait"

$clusterNodes = Get-ClusterNode

foreach($clusterNode in $clusterNodes) {
    Write-Host -ForegroundColor Cyan "--Gathering info from cluster node '$($clusterNode.Name)'..."
    $vms = Get-VM -ComputerName $clusterNode.Name

    foreach($vm in $vms) {
	    $vhds = Get-VHD -VmId $vm.vmId -ComputerName $clusterNode.Name
        ...
        $null = $vmOutput.Add((New-Object PSObject -Property @{'VM_Name' = $vm.Name; 'VM_State' = $vm.State; 'Host_Name'=$vm.ComputerName; 'IsClustered'=$vm.IsClustered; 'vCPU'=$vm.ProcessorCount; 'RAM_GB'=($vm.MemoryStartup/1Gb -as [int]); `
